### GENERATE SUPPLEMENTARY FIGURE 6C ##############################################################
# create metastatic boxplot of ER and APOBEC3B overlap with ecDNA
...
for (tf in c('APOBEC3B','ESR1')) {
	...
	real <- calculate_overlap(bed,  pecdna)
	real$tf <- tf
	real$type <- 'cyclic'
	null <- calculate_overlap(bed,  pnoncyclic)
	null$tf <- tf
	null$type <- 'noncyclic'
	
	res[[tf]] <- rbind(real, null)
	...
	stats[[tf]] <- data.frame(
		tf = tf,
		es = median(real$FreqNorm)-median(null$FreqNorm),
		p = statstmp$p.value
		)
}
...
create.boxplot(
	...
	text.labels = c(
		'APOBEC3B','ESR1',
		expression('ES=0.5'), expression('P=0.48'),
		expression('ES=0.6'), expression('P=0.87')
		),
	add.rectangle = TRUE,
	...
	ytop.rectangle = 35,
	#ytop.rectangle = 10,
	col.rectangle = c('springgreen4','mediumpurple2'),
	...
	text.cex = 1,
	text.fontface = 'plain',
	filename = 'supplementary_figure6c.pdf',
	resolution = 300
	)
### SAVE ##########################################################################################
...
write.table(
  res,
  file = file.path(main_repo_path, 'data', 'SupplementaryFigure6c_sourcetable.txt'),
  sep = '\t',
  row.names = FALSE,
  quote = FALSE
  )