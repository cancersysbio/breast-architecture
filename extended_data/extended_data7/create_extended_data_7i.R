### CREATE EXTENDED DATA 7I #######################################################################
# create boxplot of difference in R-loops with and without A3B knockdown
...
# set file names
dripseq_files <- list()
dripseq_files[['ko']] <- file.path(main_repo_path, 'data', 'GSE148581_DRIP_KO_PMA_6h_Called_Peaks.txt.gz')
dripseq_files[['wt']] <- file.path(main_repo_path, 'data', 'GSE148581_DRIP_WT_PMA_6h_Called_Peaks.txt.gz')
dripseq_files[['ko_baseline']] <- file.path(main_repo_path, 'data', 'GSE148581_DRIP_KO_DMSO_Called_Peaks.txt.gz')
dripseq_files[['wt_baseline']] <- file.path(main_repo_path, 'data', 'GSE148581_DRIP_WT_DMSO_Called_Peaks.txt.gz')
...
create.boxplot(
	...
	add.text = TRUE,
	text.labels = c(
		expression('ES=17.5'), expression('P=2.27x10'^'-4'),
		expression('ES=-3'), expression('P=0.50')
		),
...
write.table(
	plot_data_both,
	file = file.path(main_repo_path, 'data', 'ExtendedData7i_sourcetable.txt'),
	sep = '\t',
	row.names = FALSE,
	quote = FALSE
	)
